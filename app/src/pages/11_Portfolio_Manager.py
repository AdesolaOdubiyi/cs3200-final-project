...
# ------------------------------------------------------------------------------
st.set_page_config(
    page_title="Portfolio Manager - Stratify",
    page_icon="📂",
    layout="wide",
)
...
st.markdown(
    ...
    <div style="padding: 1.5rem 0 1rem 0;">
        <h1 style="font-size: 2.5rem; color: var(--primary); margin-bottom: 0.25rem;">
            Portfolio Manager
        </h1>
...
tab1, tab2, tab3 = st.tabs(
    ["Active Portfolios", "Trade Execution", "Create New Model"]
)
...
with tab1:
    ...
    with col1:
        ...
        with st.expander("View/Edit Notes for AAPL"):
            st.text_area(
                "Analyst Notes",
                (
                    "Strong Q3 earnings expected. Maintaining overweight position "
                    "despite tech sector volatility. Watch for supply chain updates."
                ),
                height=100,
            )
    ...
    with col2:
        ...
        st.markdown(
            """
            <div class="card">
                <h4 style="color:var(--text-secondary); font-size:0.9rem; text-transform:uppercase;">
                    Total AUM
                </h4>
                <h2 style="color:var(--success); font-family:var(--font-mono);">
                    $955,410
                </h2>

                <div style="height:10px; background:var(--bg-border); border-radius:5px;
    ...
    with c1:
        ...
        if st.button("Submit Order"):
            if not ticker:
                ...
                # Note: `price` is not used in this demo for Limit/Stop orders
                st.success(
                    f"✅ Order Executed: {side} {quantity} {ticker} @ {order_type}"
                )
    ...
    with c2:
        ...
        st.markdown(
            ...
            <div style="font-size:0.9rem;">
                <div style="padding:0.5rem 0; border-bottom:1px solid var(--bg-border);
...
with tab3:
    ...
    with st.form("new_portfolio_form"):
        ...
        with col_a:
            ...
            p_capital = st.number_input(
                "Initial Capital ($)",
                value=1_000_000,
            )

        with col_b:
            p_benchmark = st.selectbox(
                "Benchmark",
