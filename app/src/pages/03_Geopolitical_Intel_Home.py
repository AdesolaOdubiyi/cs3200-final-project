...
with c1:
    ...
    events = [
        {"region": "Eastern Europe", "event": "Energy supply chain disruption reported", "impact": "High", "sector": "Energy"},
        {"region": "South East Asia", "event": "Trade agreement negotiations stalled", "impact": "Medium", "sector": "Manufacturing"},
        {"region": "North America", "event": "New fiscal policy announcement expected", "impact": "Low", "sector": "Finance"},
    ]
    
    for e in events:
        risk_class = "risk-level-high" if e['impact'] == "High" else "risk-level-med" if e['impact'] == "Medium" else "risk-level-low"
        st.markdown(
            f"""
            <div class="geo-card">
                ...
                <div style="font-size:1.1rem; font-weight:bold; margin-bottom:0.5rem;">{e['event']}</div>
...
with c2:
    ...
    if st.button("Download Full Report", use_container_width=True):
        from stratify_loader import show_stratify_loader  # lazy import to avoid circular
